...
const PricingCard = ({most,title,semiTitle,pricing,desc}) => {
  return (
    <div className={`py-10 px-10 bg-[#ffffff]  relative ${most?'h-[630px] ':'h-[550px] '}`}>
        {most && (
        <button
          className="bg-[#0055D2] absolute top-3 right-3 text-white text-center px-5 py-2 mb-44"
        >
          {most}
        </button>
      )}
        <div className={`text-center ${most?'mt-12':''}`}>
            ...
            <p className={`text-sm text-[#616161] mb-4   flex justify-center   `}>{semiTitle}</p>
            <h1 className='text-[#0055D2] text-2xl font-bold'>{pricing} ر.س <sup>/مرة واحدة</sup></h1>
